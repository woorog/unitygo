...
namespace Unity.Services.Multiplayer
{
    ...
    internal class LobbyHandler : ILobbyHandler
    {
        ...
        internal void OnLobbyChanged(ILobbyChanges changes)
        {
            ...
            {
                OnPlayerLeft(changes.PlayerLeft.Value);
            }
        ...
        internal void OnPlayerLeft(List<int> players)
        {
            if (PlayerLeft == null) return;

            foreach (var playerIndex in players)
            {
                // note(luke): were applying this before we update the local lobby, so player should exist.
                if (Lobby?.Players?.Count > playerIndex)
                {
